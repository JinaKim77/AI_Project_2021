...
public class GameWindow extends Application{
	...
	public static int updatedCol;
	private boolean gameOver = false;
	private boolean pause = false;
	private GameModel[] enemies = new GameModel[5];
	private Node current;
	private Node[][] maze;
	public static int health = 100; //Player's health when the game starts
	public static int damage = 0; //Player's damage when the game starts
	
	//=========Fuzzy Implementation===========================
	FuzzyLogicImp fuzzyImp = new FuzzyLogicImp();
	double risk; //= fuzzyImp.getRisk(currentRow,currentCol);	
	//========================================================
	...
	public void placePlayer(){  //Place the main player character	
	...
	public void setGameOver(boolean gameOver) {
		...
		if (this.gameOver){
			for (int i = 0; i < enemies.length; i++){
				enemies[i].setPause(true);
			}
		} else{
			for (int i = 0; i < enemies.length; i++){
				enemies[i].setPause(false);
			}
		}
	}
